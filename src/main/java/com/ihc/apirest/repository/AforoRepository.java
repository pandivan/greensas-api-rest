...
public interface AforoRepository extends JpaRepository<Aforo, Long>
{
     ...
     */
    @Query(value = "select count(1) from dimension.aforo where fecha_ingreso is not null and fecha_ingreso >= CURRENT_DATE", nativeQuery = true)
     ...
     */
    @Query(value = "select count(1) from dimension.aforo where fecha_salida is not null and fecha_salida >= CURRENT_DATE", nativeQuery = true)
